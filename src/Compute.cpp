...
void nts::Compute::Display(nts::Factory factory, std::size_t ticks)
{
    ...
    std::map<std::string, std::shared_ptr<IComponent>> inputs;
    std::map<std::string, std::shared_ptr<IComponent>> output;
    for (auto &chip : chipsets) {
        if (dynamic_cast<CInput *>(chip.second.get())) {
            inputs[chip.first] = chip.second;
        }
        if (dynamic_cast<COutput *>(chip.second.get())) {
